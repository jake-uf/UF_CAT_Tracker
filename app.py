...
def map_display():
    ...
    if form.validate_on_submit():
        ...
        if form.doc.data:
            ...
            form.doc.data.save('uploads/' + name + '_' + str_now + ".png")
            filepath = name + '_' + str_now + ".png"
...
@app.route('/home')
def home():
    animal_list = ['Tenders', 'Apollo', 'Ryan the Engineering Cat', 'Campus Cat #4']
    return render_template('home.html', animals=animal_list)


def get_location(ip):
    geoip_data = simple_geoip.get_geoip_data()
    if geoip_data['ip'] == '127.0.0.1':
        return 29.6445690824364, -82.34392573073474
    else:
...
def appendDatabase(name, lat, long, icon, image):
    catMarkers.append(cat(name, (lat,long), icon, image))
